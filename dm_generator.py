...
DM_INTERFACE = [
    ...
    # ==================== AI/Yolo 接口 ====================
    ("LoadAi", [("file", "str", "AI模块路径，如：c:\\ai.module 或 ai.module(相对路径)。仅支持正式版本")], "int", "加载AI模块(从后台下载)。加载是全进程生效，同进程只需加载一次。加载过的进程退出时需手动调用TerminateProcess(-1)结束自己，以免造成资源泄露",
     {1: "成功", -1: "打开文件失败", -2: "内存初始化失败(正式版本出现可联系作者)", -3: "参数错误", -4: "加载错误", -5: "AI模块初始化失败", -6: "内存分配失败"}),
    ("UnLoadAi", [], "int", "卸载AI模型",
     {0: "失败", 1: "成功"}),
    ("LoadAiMemory", [
        ("data", "int", "AI模块在内存中的地址"),
        ("size", "int", "AI模块在内存中的大小")
    ], "int", "从内存加载AI模块。加载是全进程生效，同进程只需加载一次。加载过的进程退出时需手动调用TerminateProcess(-1)结束自己，以免造成资源泄露",
     {1: "成功", -1: "打开文件失败", -2: "内存初始化失败(正式版本出现可联系作者)", -3: "参数错误", -4: "加载错误", -5: "AI模块初始化失败", -6: "内存分配失败"}),
    ("AiEnableFindPicWindow", [("enable", "int", "0=关闭调试窗口,1=开启调试窗口")], "int", "设置是否在调用AiFindPicXX系列接口时弹出找图结果调试窗口",
     {0: "失败", 1: "成功"}),
    ("AiFindPic", [
        ("x1", "int", "查找区域左上角X坐标"),
        ("y1", "int", "查找区域左上角Y坐标"),
        ...
        ("sim", "float", "相似度，取值范围0.1-1.0"),
        ("dir", "int", "查找方向：0=从左到右从上到下,1=从左到右从下到上,2=从右到左从上到下,3=从右到左从下到上")
    ], "str", "使用AI模块查找指定区域内的图片(位图必须是24位色格式,支持透明色,当图像上下左右4个顶点颜色一样时该颜色作为透明色处理)。比传统FindPic效果更好，不需要训练。只返回第一个找到的XY坐标。需要ai.module 4.0及其之后的版本",
     {"-1|-1": "未找到", "其他": "x|y 格式，如: 100,200"}),
    ("AiFindPicEx", [
        ("x1", "int", "查找区域左上角X坐标"),
        ("y1", "int", "查找区域左上角Y坐标"),
        ...
        ("sim", "float", "相似度，取值范围0.1-1.0"),
        ("dir", "int", "查找方向：0=从左到右从上到下,1=从左到右从下到上,2=从右到左从上到下,3=从右到左从下到上")
    ], "str", "使用AI模块查找指定区域内的图片(位图必须是24位色格式,支持透明色,当图像上下左右4个顶点颜色一样时该颜色作为透明色处理)。比传统FindPicEx效果更好，不需要训练。返回所有找到的图像坐标。需要ai.module 4.0及其之后的版本",
     {"": "未找到", "其他": "id,x,y|id,x,y... 格式，如: 0,100,20|2,30,40（最多返回1500个左右）"}),
    ("AiFindPicMem", [
        ("x1", "int", "查找区域左上角X坐标"),
        ("y1", "int", "查找区域左上角Y坐标"),
        ("x2", "int", "查找区域右下角X坐标"),
        ...
        ("sim", "float", "相似度，取值范围0.1-1.0"),
        ("dir", "int", "查找方向：0=从左到右从上到下,1=从左到右从下到上,2=从右到左从上到下,3=从右到左从下到上")
    ], "str", "使用AI模块从内存中查找指定区域内的图片(位图必须是24位色格式,支持透明色,当图像上下左右4个顶点颜色一样时该颜色作为透明色处理)。比传统FindPicMem效果更好，不需要训练。只返回第一个找到的XY坐标。需要ai.module 4.0及其之后的版本",
     {"-1|-1": "未找到", "其他": "x|y 格式，如: 100,200"}),
    ("AiFindPicMemEx", [
        ("x1", "int", "查找区域左上角X坐标"),
        ("y1", "int", "查找区域左上角Y坐标"),
        ...
        ("sim", "float", "相似度，取值范围0.1-1.0"),
        ("dir", "int", "查找方向：0=从左到右从上到下,1=从左到右从下到上,2=从右到左从上到下,3=从右到左从下到上")
    ], "str", "使用AI模块从内存中查找指定区域内的图片。比传统FindPicMemEx效果更好，不需要训练。返回所有找到的图像坐标。需要ai.module 4.0及其之后的版本",
     {"": "未找到", "其他": "id,x,y|id,x,y... 格式，如: 0,100,20|2,30,40（最多返回1500个左右）"}),
    ("AppendPicAddr", [
        ("pic_info", "str", "已有的图片数据地址集合，首次调用传空字符串"),
        ("addr", "int", "24位位图资源在内存中的首地址(十进制)"),
        ("length", "int", "位图资源在内存中的长度(十进制)")
    ], "str", "追加图片数据地址到集合中，用于AiFindPicMem/AiFindPicMemEx",
     {"": "失败", "其他": "追加后的图片数据地址集合"}),
    ("AiYoloUseModel", [("model_index", "int", "模型序号，从0开始")], "int", "设置当前使用的Yolo模型",
     {0: "失败", 1: "成功"}),
    ("AiYoloDetectObjects", [
        ("x1", "int", "检测区域左上角X坐标"),
        ("y1", "int", "检测区域左上角Y坐标"),
    ...
    ], "str", "使用Yolo模型在指定范围内检测对象。需要先调用AiYoloUseModel加载模型。模块内部是全局的，多线程同时执行会排队",
     {"": "未检测到", "其他": "类名,置信度,x,y,w,h|... 格式"}),
    ...
    ], "int", "使用Yolo模型检测对象并把结果输出到BMP图像数据(用于二次开发)。需要先调用AiYoloUseModel加载模型。模块内部是全局的，多线程同时执行会排队",
     {0: "失败", 1: "成功"}),
    ("AiYoloDetectObjectsToFile", [
        ...
        ("mode", "int", "0=绘制文字包含置信度,1=绘制文字不包含置信度")
    ], "int", "使用Yolo模型检测对象并把结果输出到指定的BMP文件。需要先调用AiYoloUseModel加载模型。模块内部是全局的，多线程同时执行会排队",
     {0: "失败", 1: "成功"}),
    ("AiYoloFreeModel", [("index", "int", "模型序号，最多支持20个，从0开始")], "int", "卸载指定的Yolo模型。模型内部是全局的，调用时需确保没有其他接口访问此模型",
     {0: "失败", 1: "成功"}),
    ("AiYoloObjectsToString", [("objects", "str", "AiYoloDetectObjects或AiYoloSortsObjects的返回值")], "str", "把Yolo检测结果中的class信息按顺序连接输出字符串",
     {"": "空结果", "其他": "class信息连接后的字符串"}),
    ("AiYoloSetModel", [
        ("index", "int", "模型序号，最多支持20个，从0开始"),
        ("file", "str", "模型文件名，如：xxxx.onnx或xxxx.dmx"),
        ("pwd", "str", "模型密码，仅对dmx格式有效")
    ], "int", "从文件加载指定的Yolo模型。加载onnx时需确保同名class文件在同目录下(如xxxx.onnx需有xxxx.class)。模型内部是全局的，多线程同时执行会排队",
     {0: "失败", 1: "成功"}),
    ("AiYoloSetModelMemory", [
        ...
        ("size", "int", "dmx模型的大小"),
        ("pwd", "str", "dmx模型的密码")
    ], "int", "从内存加载指定的Yolo模型(仅支持dmx格式)。模型内部是全局的，多线程同时执行会排队",
     {0: "失败", 1: "成功"}),
    ("AiYoloSetVersion", [("ver", "str", "Yolo版本信息，目前可选值：v5-7.0。需要在加载Ai模块后第一时间调用")], "int", "设置Yolo的版本",
     {0: "失败", 1: "成功"}),
    ("AiYoloSortsObjects", [
        ("objects", "str", "AiYoloDetectObjects的返回值"),
        ("height", "int", "行高信息，用于确定两个检测框是否处于同一行(Y坐标相差绝对值小于此行高则认为是同一行)")
    ], "str", "把Yolo检测结果按照从上到下、从左到右排序",
     {"": "空结果", "其他": "类名,置信度,x,y,w,h|... 格式"}),
    ("SetDisplayInput", [("input_type", "str", "输入类型：screen=屏幕,pic:文件名=从图片输入")], "int", "设置显示输入源",
     {0: "失败", 1: "成功"}),
